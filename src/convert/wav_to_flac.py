...
def convert_wav_to_flac(wav_file, overwrite=False):
    ...
    try:
        ...
        if os.path.exists(flac_file):
            ...
            try:
                ffmpeg.input(wav_file).input(flac_file).filter(
                    "amix", inputs=2, duration="first", dropout_transition=3
                ).output("null").run()
                # Delete the original WAV file
                os.remove(wav_file)
                logging.info(
                    f"Successfully converted {wav_file} to {flac_file} and deleted the original WAV file."
                )
            except ffmpeg.Error as e:
                logging.error(
                    f"Audio streams differ between {wav_file} and {flac_file}. Conversion failed. Error: {e}"
                )
        else:
