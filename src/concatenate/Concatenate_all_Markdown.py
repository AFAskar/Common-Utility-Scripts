...
def concatenate_markdown_files(
    ...
    Parameters:
    directory (str | Path): The directory to search for markdown files.
        ...
        for md_file in markdown_files:
            with open(md_file, "r", encoding="utf-8") as infile:
                ...
                if char_limit and current_char_count + file_length > char_limit:
                    ...
                    current_char_count = 0
                    outfile = open(
                        f"concatenated_{file_index}.md", "w", encoding="utf-8"
                    )

                if include_filenames:
                    if use_relative_path:
                        filename = os.path.relpath(md_file, directory).replace(
                            os.sep, "\\\\"
                        )
                    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Concatenate all markdown files in a directory."
    )
    parser.add_argument(
        "directory", type=str, help="The directory to search for markdown files."
    )
    parser.add_argument(
        "--ignore-tags", action="store_true", help="Ignore lines with tags."
    )
    parser.add_argument(
        "--include-filenames", action="store_true", help="Include filenames as headers."
    )
    parser.add_argument(
        "--use-relative-path",
        action="store_true",
    ...
    parser.add_argument(
        "--char-limit", type=int, help="Character limit for each output file."
    )
